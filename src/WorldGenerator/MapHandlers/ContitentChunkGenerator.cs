...
namespace WorldGenerator.MapHandlers{
    public class ContinentChunkGenerator : ChunkGenerator{
        private readonly DistanceRatioCalculator
            _firstDistanceRatioCalculator; //= new DistanceRatioCalculator(400, 200);
        ...
        public ContinentChunkGenerator(Map map) : base(map, -0.2f){
        ...
        protected override double CalculateBlockValue(Position position){
            ...
            tmp *= _firstDistanceRatioCalculator.GetValue(position);
            return tmp;
        }
